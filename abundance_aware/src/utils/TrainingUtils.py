...
from abundance_aware.src.utils.Evaluator import Evaluator
...
def gen_num_sent(start, model, num_sent, tokenizer, bad_words=None):
    gen_sent = [generate(sent, model, tokenizer, bad_words_ids=bad_words, num_return_sequences=num_sent) for sent in
                start]
    gen_sent = [
        torch.cat([sent, torch.ones(num_sent, 512 - sent.shape[1], dtype=torch.long) * tokenizer.pad_token_id], dim=1)
