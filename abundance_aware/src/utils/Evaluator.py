...
class Evaluator:
    ...
    def eval(self):
        ...
        metrics_layer = dict(
            eval_single_label(predictions[label], actual_sources[label], self.thresholds, self.nafill) for label in
            labels)
        ...
        avg_metrics_layer = pd.DataFrame(np.concatenate([np.expand_dims(metrics_layer[label][avg_metrics].to_numpy(), 2)
                                                         for label in labels], axis=2).mean(axis=2),
                                         columns=avg_metrics)
...
def eval_single_label(predictions, actual_sources, thresholds, nafill):
    ...
    conf_matrix = metrics['t'].apply(
        lambda T: confusion_matrix(actual_sources, pred_source.iloc[int(T * num_thresholds), :], labels=[0, 1]).ravel())
